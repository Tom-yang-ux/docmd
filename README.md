...
当检测到 C/D 字段时，经 **AI 汇总提问 → 用户确认门控** 后生成最终 `final.md` 与审计 `evidence.json`；
没有 C/D 字段则直接生成最终文件。
...
- **默认原则**：C/D 字段保留在 Markdown；AI 读完整文档后先汇总提问；用户确认前禁止后续处理；用户确认后才生成可用最终文件。没有 C/D 时不进入确认页，直接生成最终文件。
...
- 直接将 PDF、图片、Word、Excel 或 PPT 拖到小熊身上，它会在转换期间播放工作动画；无 C/D 时自动导出，存在 C/D 时双击小熊打开确认面板。
- 单击小熊会触发抚摸与开心动画；连续 10 次抚摸解锁小女孩桌宠。拖动小女孩与小熊相碰，会播放终极彩蛋视频。
- 点击小熊桌面下方的饼干，会出现掉落动画，小熊随后哭泣。
- 双击小熊可打开完整管理面板（输出路径、待确认、AI 与模型设置）。
...
界面中的“选择输出位置”可更改并记住上述 `output/` 的位置；之后生成的待确认文件、最终 Markdown 与审计文件都会保存到所选目录。
